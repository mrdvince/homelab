...
Infrastructure-as-code for my homelab running on Proxmox. This repo provisions Kubernetes clusters using Talos, deploys applications via ArgoCD and Helmfile, and manages everything through Terragrunt. 

Secrets are encrypted with SOPS using age keys.

## How it works

The infrastructure layer uses Terragrunt to provision VMs on Proxmox and bootstrap Talos clusters. Once a cluster is up, ArgoCD takes over and deploys applications using an app-of-apps pattern. Each application has its own helmfile that gets rendered by the ArgoCD Helmfile plugin.

Container images are synced to a private GitLab registry, and Helm charts are published to GitLab's package registry. This keeps everything internal and avoids rate limits from public registries.
...
│   ├── avalon/              # proxmox node
│   │   └── clusters/aion/   # kubernetes cluster
...
1. The infrastructure starts with Terragrunt in `infrastructure/`. Run `terragrunt apply --all` from a node directory to provision VMs and bootstrap the cluster.
